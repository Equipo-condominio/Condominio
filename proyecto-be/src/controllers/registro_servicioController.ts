...
import { Registro_servicio } from 'src/models/registro_servicio';

@Controller('registro_servicio')
export class Registro_servicioController {
    @Post()
    async create(@Body() body: Registro_servicio,  @Headers() headers): Promise<any> {
        let connection = await SqlHelper.connect(DB_HOST, DB_USERNAME, DB_PASSWORD, DB_PORT, DB_DATABASE);
        let command: string = `INSERT INTO public_registro_servicio(nombre, monto, fecha, status, cancelado_bs, cancelado_dollar, status_eliminado, id_tasa_dia, id_public_condominio, creador) VALUES ('${body.nombre}', ${body.monto}, '${body.fecha}', '${body.status}', ${body.cancelado_bs || null}, ${body.cancelado_dollar || null},'${body.status_eliminado || null}', ${body.id_tasa_dia || null}, ${body.id_public_condominio || null}, ${headers.authorization})`;
    ...
    @Post('edit')
    async edit(@Body() body: Registro_servicio): Promise<string> {
        let connection = await SqlHelper.connect(DB_HOST, DB_USERNAME, DB_PASSWORD, DB_PORT, DB_DATABASE);
        
        const lastRegistro: any = {};
        await SqlHelper.run(connection, `SELECT *, id_public_registro_servicio as id FROM public_registro_servicio WHERE id_public_registro_servicio=${body.id}`, lastRegistro);
    ...
    async findOne(@Query('id') id): Promise<any> {
        ...
        const output: any = {};
        await SqlHelper.run(connection, `SELECT *, id_public_registro_servicio as id FROM public_registro_servicio WHERE id_public_registro_servicio=${id}`, output);
    ...
    async findAll(@Query('pageNumber') pageNumber, @Query('pageSize') pageSize, @Headers() headers): Promise<any> {
        ...
        const count: any = {};
        await SqlHelper.run(connection, `SELECT COUNT(*) FROM public_registro_servicio WHERE public_registro_servicio.status_eliminado != 2 AND public_registro_servicio.creador=${headers.authorization}`, count);
        
        const items: any = {};
        await SqlHelper.run(connection, `SELECT *, id_public_registro_servicio as id FROM public_registro_servicio WHERE public_registro_servicio.status_eliminado != 2 AND public_registro_servicio.creador=${headers.authorization} LIMIT ${pageNumber* pageSize}, ${pageSize}`, items);
    ...
    async delete(@Body() body: any): Promise<string> {
        let connection = await SqlHelper.connect(DB_HOST, DB_USERNAME, DB_PASSWORD, DB_PORT, DB_DATABASE);
        let command: string = `DELETE FROM public_registro_servicio WHERE id_public_registro_servicio IN (${body.id.join(',')})`;
        console.log(command);
        const output: any = {};
        await SqlHelper.run(connection, command, output);
