...
export class EstadosController {
    ...
    async findOne(@Query('id') id): Promise<any> {
        ...
        const output: any = {};
        await SqlHelper.run(connection, `SELECT * FROM estados WHERE id_estado=${id}`, output);
    ...
    async findAll(@Query('pageNumber') pageNumber, @Query('pageSize') pageSize): Promise<any> {
        ...
        const count: any = {};
        await SqlHelper.run(connection, `SELECT COUNT(*) FROM estados`, count);
        
        const items: any = {};
        await SqlHelper.run(connection, `SELECT *, id_estado as id FROM estados LIMIT ${pageNumber* pageSize}, ${pageSize}`, items);
